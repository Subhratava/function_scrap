...
from table_copy import copy_excel_tables_to_docx
...
DEFAULT_TABLE_COPY_OUTPUT_FILE = "Table_Copier_Output.docx"
...
# Easy to modify: each entry maps one UI section to one DOCX table placeholder.
TABLE_COPIER_SECTIONS = [
    {
        "section": "Summary",
...
def ensure_excel_source(uploaded_excel) -> Path | None:
    if uploaded_excel is None:
        return get_uploaded_excel_path()

    excel_path = save_uploaded_template(uploaded_excel)
    st.session_state.uploaded_excel_path = str(excel_path)
    return excel_path


def get_uploaded_excel_path() -> Path | None:
    excel_path = st.session_state.get("uploaded_excel_path")
    if not excel_path:
        return None
...
def get_excel_sheet_dimensions(excel_path: Path) -> dict[str, str]:
    workbook = pd.ExcelFile(excel_path)
    dimensions: dict[str, str] = {}
    for sheet_name in workbook.sheet_names:
        frame = pd.read_excel(excel_path, sheet_name=sheet_name, header=None)
        row_count, col_count = frame.shape
        if row_count == 0 or col_count == 0:
            dimensions[sheet_name] = "A1:A1"
            continue

        col_name = ""
        col_number = col_count
        while col_number:
            col_number, remainder = divmod(col_number - 1, 26)
            col_name = chr(65 + remainder) + col_name
        dimensions[sheet_name] = f"A1:{col_name}{row_count}"
    return dimensions
...
homepage_tab, table_copier_tab, help_tab = st.tabs(["Homepage", "Table Copier", "Help"])
...
with table_copier_tab:
    st.subheader("Table Copier")

    excel_path = get_uploaded_excel_path()
    if excel_path is None:
        st.warning("Please upload an Excel workbook in the Homepage tab first.")
    else:
        st.caption(f"Using workbook uploaded in Homepage: {excel_path.name}")

        try:
            workbook = pd.ExcelFile(excel_path)
            sheet_names = workbook.sheet_names
            sheet_dimensions = get_excel_sheet_dimensions(excel_path)
        except Exception as exc:
            st.error(f"Could not read uploaded workbook: {exc}")
            sheet_names = []
            sheet_dimensions = {}
        ...
        if sheet_names:
            ...
            for section_config in TABLE_COPIER_SECTIONS:
                section_name = section_config["section"]
                default_sheet = section_config["default_sheet"]
                placeholder = section_config["placeholder"]
                default_range = section_config["default_range"]

                with st.container(border=True):
                    st.markdown(f"#### {section_name}")
                    st.caption(f"Selected range will replace {{{{{placeholder}}}}}.")
                    ...
                    sheet_col, range_col = st.columns([1, 1])
                    with sheet_col:
                        selected_sheet = st.selectbox(
                            f"Sheet for {section_name}",
                            sheet_names,
                            index=default_index,
                            key=f"table_copier_sheet_{placeholder}",
                        )

                    detected_range = sheet_dimensions.get(
                        selected_sheet,
                        default_range,
                    )
                    range_key = f"table_copier_range_{placeholder}"
                    if range_key not in st.session_state:
                        st.session_state[range_key] = detected_range

                    with range_col:
                        selected_range = st.text_input(
                            f"Excel range for {section_name}",
                            key=range_key,
                            help=f"Detected used range for selected sheet: {detected_range}",
                        )

                    if selected_sheet and selected_range:
                        table_map[selected_sheet] = (selected_range.strip(), placeholder)
                        selected_sheet_names.append(selected_sheet)

            if len(selected_sheet_names) != len(set(selected_sheet_names)):
                st.warning(
                    "Two sections currently use the same sheet. The current internal "
                    "map is keyed by sheet name, so please choose different sheets."
                )

            max_width_ratio = st.slider(
                "Table width inside page",
                min_value=0.50,
                max_value=1.00,
                value=1.00,
                step=0.05,
                help="1.00 uses the full printable page width; lower values leave more side space.",
            )

            create_tables_clicked = st.button(
                "Create DOCX with copied tables",
                type="primary",
                use_container_width=True,
                disabled=not bool(table_map)
                or len(selected_sheet_names) != len(set(selected_sheet_names)),
            )

            if create_tables_clicked:
                try:
                    output_path = Path(DEFAULT_TABLE_COPY_OUTPUT_FILE).expanduser().resolve()
                    with st.spinner("Copying formatted Excel tables into DOCX..."):
                        copied_path = copy_excel_tables_to_docx(
                            BASE_TEMPLATE_PATH,
                            table_map,
                            excel_path,
                            output_path,
                    ...
                    st.session_state.table_copier_output_path = str(copied_path)
                    st.success("Table copier DOCX created successfully.")
                except Exception as exc:
                    st.error(f"Failed to create table copier DOCX: {exc}")
                    st.code(traceback.format_exc())

            copied_output = st.session_state.get("table_copier_output_path")
            if copied_output:
                copied_output_path = Path(copied_output).expanduser().resolve()
                if copied_output_path.exists():
                    with copied_output_path.open("rb") as f:
                        st.download_button(
                            label="Download table copier DOCX",
                            data=f.read(),
                            file_name=copied_output_path.name,
                            mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                            type="primary",
                            width="stretch",
                            key="download_table_copier_docx",
                        )
