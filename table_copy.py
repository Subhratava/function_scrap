...
DEFAULT_TEST_MAP = {
    "summary": [("A1:G8", "table1", False)],
    "sales": [("A1:E6", "table2", False)],
}

TableSpec = tuple[str, str | int] | tuple[str, str | int, bool]
...
def copy_excel_tables_to_docx(
    doc: Document,
    table_map: Mapping[str, list[TableSpec] | TableSpec],
    excel_path: str | Path,
    ...
    for sheet_name, specs in table_map.items():
        if sheet_name not in workbook.sheetnames:
            continue

        worksheet = workbook[sheet_name]

        if isinstance(specs, tuple) and len(specs) in {2, 3}:
            specs = [specs]

        for spec in specs:
            cell_range, table_ref, landscape_mode = _normalize_table_spec(spec)
            ...
            inserted_table = _build_docx_table_from_excel_range(
                ...
                section_for_sizing=_landscape_section_proxy(document.sections[0])
                if landscape_mode
                else None,
            )

            was_replaced = _replace_placeholder_with_table(
                document,
                placeholder,
                inserted_table,
            )
            if was_replaced and landscape_mode:
                make_table_landscape(document, inserted_table)
...
def _normalize_table_spec(spec: TableSpec) -> tuple[str, str | int, bool]:
    if not isinstance(spec, (list, tuple)) or len(spec) not in {2, 3}:
        raise ValueError(
            "Each table_map entry must be (excel_range, table_ref) or "
            "(excel_range, table_ref, landscape_mode)."
        )

    cell_range = str(spec[0])
    table_ref = spec[1]
    landscape_mode = bool(spec[2]) if len(spec) == 3 else False
    return cell_range, table_ref, landscape_mode


def _landscape_section_proxy(section):
    page_width = max(section.page_width, section.page_height)
    page_height = min(section.page_width, section.page_height)
...
def make_table_landscape(document: Document, table) -> None:
    """
    Wrap one inserted table in its own landscape section.

    Section properties in DOCX describe the content before that section break.
    The paragraph before the table closes the previous portrait section, while
    the paragraph after the table closes the landscape section that contains
    only the table.
    """
    section = document.sections[0]
    ...
    before_paragraph = _section_break_paragraph(section, landscape=False)
    after_paragraph = _section_break_paragraph(section, landscape=True)
    table_element.addprevious(before_paragraph)
    table_element.addnext(after_paragraph)


def _section_break_paragraph(section, landscape: bool):
    paragraph = OxmlElement("w:p")
    paragraph_properties = OxmlElement("w:pPr")
    paragraph_properties.append(_section_properties(section, landscape=landscape))
    paragraph.append(paragraph_properties)
    return paragraph


def _section_properties(section, landscape: bool):
    section_properties = deepcopy(section._sectPr)
    page_size = section_properties.find(qn("w:pgSz"))
    if page_size is None:
        page_size = OxmlElement("w:pgSz")
        section_properties.insert(0, page_size)

    width = int(section.page_width)
    height = int(section.page_height)
    if landscape:
        page_size.set(qn("w:orient"), "landscape")
        page_size.set(qn("w:w"), str(_emu_to_twips(max(width, height))))
        page_size.set(qn("w:h"), str(_emu_to_twips(min(width, height))))
    else:
        page_size.set(qn("w:orient"), "portrait")
        page_size.set(qn("w:w"), str(_emu_to_twips(min(width, height))))
        page_size.set(qn("w:h"), str(_emu_to_twips(max(width, height))))
...
def _build_docx_table_from_excel_range(
    ...
    _apply_table_widths(table, widths)
    _apply_table_outside_border(table)
    ...
    for row_offset, excel_row in enumerate(range(min_row, max_row + 1)):
        ...
        for col_offset, excel_col in enumerate(range(min_col, max_col + 1)):
            ...
            _set_cell_text(word_cell, _format_cell_value(excel_cell))
    ...
    if fit_to_page:
        auto_resize_table_to_page(
            table,
            section_for_sizing or document.sections[0],
            max_width_ratio=max_width_ratio,
            min_column_width=min_column_width,
            min_font_size_pt=min_font_size_pt,
        )
...
def _apply_table_outside_border(table) -> None:
    tbl_pr = table._tbl.tblPr
    ...
    for side_name in ("top", "left", "bottom", "right"):
        existing = tbl_borders.find(qn(f"w:{side_name}"))
...
def _format_cell_value(excel_cell) -> str:
    value = excel_cell.value
    if value is None:
        return ""

    number_format = str(getattr(excel_cell, "number_format", "") or "")
    if number_format and number_format.lower() != "general":
        formatted_value = _format_excel_number_style(value, number_format, excel_cell)
...
def _format_excel_number_style(value: Any, number_format: str, excel_cell) -> str | None:
    format_code = _first_number_format_section(number_format)
    if not format_code:
        return None

    if isinstance(value, (datetime, date, time)) or getattr(excel_cell, "is_date", False) or is_date_format(format_code):
        return _format_excel_date_value(value, format_code)

    if not isinstance(value, (int, float, Decimal)) or isinstance(value, bool):
        return None

    decimal_value = Decimal(str(value))
    cleaned_format = _strip_format_literals(format_code)

    is_percent = "%" in cleaned_format
    if is_percent:
        decimal_value *= Decimal("100")

    decimal_places = _number_format_decimal_places(cleaned_format)
    ...
    use_grouping = "," in cleaned_format.split(".")[0]
    number_text = f"{rounded_value:,.{decimal_places}f}" if use_grouping else f"{rounded_value:.{decimal_places}f}"
...
def _first_number_format_section(number_format: str) -> str:
    return str(number_format).split(";")[0].strip()


def _strip_format_literals(format_code: str) -> str:
    without_brackets = re.sub(r"\[[^\]]+\]", "", format_code)
    without_quotes = re.sub(r'"[^"]*"', "", without_brackets)
    return re.sub(r"\\.", "", without_quotes)


def _number_format_decimal_places(format_code: str) -> int:
    if "." not in format_code:
        return 0
    decimal_part = format_code.split(".", 1)[1]
    decimal_part = re.split(r"[^0#?]", decimal_part, maxsplit=1)[0]
    return sum(1 for char in decimal_part if char in "0#?")


# def _format_excel_date_value(value: Any, format_code: str) -> str | None:
#     if isinstance(value, datetime):
#         date_value = value
#     elif isinstance(value, date):
#         date_value = datetime.combine(value, time.min)
#     elif isinstance(value, time):
#         date_value = datetime.combine(date.today(), value)
#     else:
#         return None

#     cleaned_format = _strip_format_literals(format_code).lower()
#     if re.fullmatch(r"d{1,2}-mmm", cleaned_format):
#         return date_value.strftime("%d-%b").lstrip("0")
#     if re.fullmatch(r"mmm-d{1,2}", cleaned_format):
#         return date_value.strftime("%b-%d").replace("-0", "-")
#     if re.fullmatch(r"d{1,2}/m{1,2}/yy(?:yy)?", cleaned_format):
#         year_format = "%Y" if "yyyy" in cleaned_format else "%y"
#         return date_value.strftime(f"%d/%m/{year_format}")
#     if re.fullmatch(r"m{1,2}/d{1,2}/yy(?:yy)?", cleaned_format):
#         year_format = "%Y" if "yyyy" in cleaned_format else "%y"
#         return date_value.strftime(f"%m/%d/{year_format}")
#     if "mmm" in cleaned_format and "d" in cleaned_format:
#         return date_value.strftime("%d-%b").lstrip("0")
...
def _format_excel_date_value(value: Any, format_code: str) -> str | None:
    ...
    # If Excel format is only d-mmm / dd-mmm, keep d-mmm output, e.g. 4-Jul
    if re.fullmatch(r"d{1,2}-mmm", cleaned_format):
        return date_value.strftime("%d-%b").lstrip("0")

    # If Excel format is only mmm-d / mmm-dd, still output d-mmm, e.g. 4-Jul
    if re.fullmatch(r"mmm-d{1,2}", cleaned_format):
        return date_value.strftime("%d-%b").lstrip("0")

    if re.fullmatch(r"d{1,2}/m{1,2}/yy(?:yy)?", cleaned_format):
        year_format = "%Y" if "yyyy" in cleaned_format else "%y"
        return date_value.strftime(f"%d/%m/{year_format}").lstrip("0")

    if re.fullmatch(r"m{1,2}/d{1,2}/yy(?:yy)?", cleaned_format):
        year_format = "%Y" if "yyyy" in cleaned_format else "%y"
        return date_value.strftime(f"%m/%d/{year_format}")

    # For other day + month-name formats, include year, e.g. 4-Jul-2026
    if "mmm" in cleaned_format and "d" in cleaned_format:
...
def _load_map_from_json(path: str | Path) -> dict[str, list[TableSpec]]:
    raw = json.loads(Path(path).read_text(encoding="utf-8"))
    table_map = {}
    for sheet_name, value in raw.items():
        if not isinstance(value, list):
            raise ValueError(
                f"Map value for {sheet_name!r} must be a list of [excel_range, table_ref] pairs."
            )

        specs = value
        if len(value) in {2, 3} and not isinstance(value[0], (list, tuple)):
            specs = [value]
        ...
        for spec in specs:
            if not isinstance(spec, (list, tuple)) or len(spec) not in {2, 3}:
                raise ValueError(
                    f"Map entries for {sheet_name!r} must be "
                    "[excel_range, table_ref] or [excel_range, table_ref, landscape_mode]."
                )
            if len(spec) == 3:
                table_map[sheet_name].append((str(spec[0]), spec[1], bool(spec[2])))
            else:
                table_map[sheet_name].append((str(spec[0]), spec[1]))
    return table_map
